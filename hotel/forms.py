...
class HotelForm(forms.ModelForm):
    ...
    hotel=forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control','placeholder':'Nom de l\'hôtel'}))
    dateAller=forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control date date1 datetimepicker-input','placeholder':'aaaa/mm/jj'}))
    dateRetour=forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control date date2 datetimepicker-input','placeholder':'aaaa/mm/jj'}))
    ...
    nbrChambre=forms.IntegerField(widget=forms.NumberInput(attrs={'class': 'form-control','value':'1'}))
    ...
    class Meta:
        model=Hotel
        fields=[
        'nom','prenom','email','hotel','telephone',
        'destination','ville','dateAller','dateRetour',
        'nbrAdulte','nbrEnfant','nbrChambre'
        ]
    def is_valid(self):
        valid = super(HotelForm, self).is_valid()
        if not valid:
            return valid
        ...
        if dater<=datea:
            self.add_error('dateRetour', 'Date Sortie Invalide')
        if datea<datetime.now().date():
             self.add_error('dateAller', 'Date Entré Invalide')
